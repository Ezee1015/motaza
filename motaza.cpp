...
Motaza::Motaza(QWidget *parent) : QWidget(parent), ui(new Ui::motaza)
{
  ui->setupUi(this);
  ...
  connect(_recordTimer, &QTimer::timeout, this, &Motaza::saveFrameToFile);
  connect(_popupTray.updateTimer, &QTimer::timeout, this, &Motaza::updateForPopups);
...
void Motaza::toggleAction(const Action action)
{
...
Form Motaza::smoothFreeForm(Form form)
{
...
bool Motaza::isActionActive(const Action action)
{
  switch (action) {
    case ACTION_WIDTH_1:
    case ACTION_WIDTH_2:
...
ButtonStatus Motaza::isButtonActive(const Button button)
{
...
int Motaza::buttonBehindCursor(const QPoint cursor)
{
  if (!_toolBar.show) {
...
bool Motaza::isCursorOverToolBar(const QPoint cursorPos)
{
  if (!isToolBarVisible()) {
    return false;
  }
...
bool Motaza::isCursorOverButton(const QPoint cursorPos)
{
  if (!isToolBarVisible()) {
    return false;
  }
...
void Motaza::saveStateToFile()
{
  QString filePath = getFilePath(FILE_MOTAZA);
  QFile file(filePath);
...
void Motaza::restoreStateFromFile(const QString path)
{
  QFile file(path);
  ...
  if (in.atEnd()) {
    ...
    logUser(LOG_ERROR_AND_EXIT, "", "There is data left in the Motaza file that was not loaded by the recovery algorithm (because it ended before the EOF). Please check the saving and the recovery algorithm: There may be some variables missing in the recovery and not in the saving or some variables added in the saving but not in the recovery...");
  }
}

void Motaza::createVideoFFmpeg()
{
  QString resolution;
...
ArrowHead Motaza::getArrowHead(const int x, const int y, const int width, const int height, int lineLength)
{
...
bool Motaza::isDrawingHovered(const int vectorPos)
{
...
bool Motaza::adjustFontSize(QFont *font, const QString text, const int rectWidth, const int minPointSize)
{
...
void Motaza::drawButton(QPainter *screenPainter, const Button button)
{
...
void Motaza::drawToolBar(QPainter *screenPainter)
{
...
void Motaza::closePopupUnderCursor(const QPoint cursorPos)
{
...
bool Motaza::isPressingPopup(const QPoint cursorPos)
{
...
QRect Motaza::getPopupRect(const int listPos)
{
...
void Motaza::drawPopup(QPainter *screenPainter, const int listPos)
{
...
void Motaza::drawPopupTray(QPainter *screenPainter)
{
  if (_screenOpts == SCREENOPTS_HIDE_ALL || _screenOpts == SCREENOPTS_HIDE_FLOATING) {
    return;
  }
...
void Motaza::drawStatus(QPainter *screenPainter)
{
  if (_screenOpts == SCREENOPTS_HIDE_ALL || _screenOpts == SCREENOPTS_HIDE_FLOATING) {
    return;
  }
...
ArrowHead Motaza::getFreeFormArrowHead(const Form freeForm)
{
...
QList<int> Motaza::getFreeFormWidth(const Form form)
{
...
void Motaza::drawSavedForms(QPainter *pixmapPainter)
{
...
void Motaza::drawFlashlightEffect(QPainter *painter, const bool drawToScreen)
{
...
void Motaza::drawTrimmed(QPainter *pixmapPainter)
{
...
void Motaza::drawActiveForm(QPainter *painter, bool drawToScreen)
{
  if (_screenOpts == SCREENOPTS_HIDE_ALL) {
    return;
  }
...
void Motaza::drawHandle(QPainter *painter, const QPoint point)
{
...
void Motaza::drawNode(QPainter *painter, const QPoint point)
{
  const int radius = NODE_RADIUS * _canvas.scale;
...
void Motaza::drawAllNodes(QPainter *screenPainter)
{
...
void Motaza::drawAllHandles(QPainter *screenPainter)
{
  if (_state == STATE_RESIZING_FORM) {
    return;
  }
...
void Motaza::paintEvent(QPaintEvent *event)
{
  (void) event;
...
void Motaza::removeFormBehindCursor(const QPoint cursorPos)
{
  // This is the position of the form (in the current draw mode) in the vector,
  // that is behind the cursor.
...
bool Motaza::isCursorOverNode(const QPoint cursorPos, const QPoint point)
{
  const int radius = NODE_RADIUS * _canvas.scale;
...
bool Motaza::selectHandle(const QPoint cursorPos)
{
  for (int i=0; i<_forms.size(); i++) {
...
bool Motaza::selectNode(const QPoint cursorPos)
{
  for (int i=0; i<_forms.size(); i++) {
    const Form f = _forms.at(i);
...
// The mouse pos shouldn't be fixed to the hdpi scaling
void Motaza::resizeForm(QPoint cursorPos)
{
...
// The mouse pos shouldn't be fixed to the hdpi scaling
void Motaza::moveForm(QPoint cursorPos)
{
  cursorPos = screenPointToPixmapPos(cursorPos);
...
void Motaza::mousePressEvent(QMouseEvent *event)
{
...
void Motaza::saveImage(const QPixmap pixmap, const bool toImage)
{
  if (toImage) {
...
void Motaza::mouseReleaseEvent(QMouseEvent *event)
{
  // The cursor pos is relative to the resolution of scaled monitor
  const QPoint cursorPos = event->pos();
...
void Motaza::mouseMoveEvent(QMouseEvent *event)
{
  // The cursor pos is relative to the resolution of scaled monitor
  const QPoint cursorPos   = event->pos();
  ...
  if (_state == STATE_DRAWING && _drawMode == FREEFORM) {
    // If the user moves the mouse out of Motaza and releases the mouse button,
...
// The mouse pos shouldn't be fixed to the hdpi scaling
void Motaza::updateAtMousePos(const QPoint mousePos)
{
...
void Motaza::wheelEvent(QWheelEvent *event)
{
...
QString Motaza::getFilePath(const FileType type)
{
  int fileIndex = 0;
  QString filePath;
  ...
  do {
    ...
    fileName = (_fileConfig.name.isEmpty()) ? ("Motaza " + date) : _fileConfig.name;
    if (fileIndex != 0) {
    ...
    switch (type) {
      ...
      case FILE_MOTAZA: fileName.append(_fileConfig.motazaExt); break;
    }
...
void Motaza::initFileConfig(const QString path, const QString name, const QString imgExt, const QString vidExt)
{
  // Path
  if (path.isEmpty()) {
  ...
  const char* defaultMotazaExt = "motaza";
  _fileConfig.motazaExt = defaultMotazaExt;
}
...
bool Motaza::isCursorInsideHitBox(int x, int y, int w, int h, const QPoint cursorPos, const bool isFloating)
{
...
bool Motaza::isCursorOverLine(int x, int y, int w, int h, const QPoint cursorPos)
{
...
bool Motaza::isCursorOverArrowHead(ArrowHead head, const QPoint cursorPos)
{
...
// drawings)
int Motaza::cursorOverForm(const QPoint cursorPos)
{
  int x, y, w, h;
  for (int i=0; i<_forms.size(); i++) {
    Form f = _forms.at(i);
...
void Motaza::keyPressEvent(QKeyEvent *event)
{
...
void Motaza::keyReleaseEvent(QKeyEvent *event)
{
...
void Motaza::createBlackboard(const QSize size)
{
...
void Motaza::grabImage(const QPixmap img)
{
  if (img.isNull()) {
...
void Motaza::dragPixmap(const QPoint delta)
{
  _canvas.pos += delta;
}

QPoint Motaza::centerCanvas() {
...
void Motaza::shiftPixmap(const QPoint cursorPos)
{
  // This is the maximum value for shifting the pixmap
  const QSize availableMargin = _windowSize - _canvas.size;
...
void Motaza::scalePixmapAt(const QPointF pos)
{
...
QPoint Motaza::screenPointToPixmapPos(const QPoint qpoint)
{
...
QPoint Motaza::pixmapPointToScreenPos(const QPoint qpoint)
{
  return QPoint(
...
QSize Motaza::pixmapSizeToScreenSize(const QSize qsize)
{
  return QSize(
...
void Motaza::drawDrawnPixmap(QPainter *painter)
{
...
bool Motaza::isDisabledMouseTracking()
{
...
bool Motaza::isTextEditable(const QPoint cursorPos)
{
...
void Motaza::logUser(const LogUrgency type, QString popupMsg, const char *fmt, ...)
{
  FILE *output;
  bool exitApp = false;
...
void Motaza::getSimpleFormPosition(const Form &userObj, int *x, int *y, int *w, int *h, const bool posRelativeToScreen)
{
