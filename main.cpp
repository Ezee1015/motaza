...
void help(const char *errorMsg)
{
  ...
  if (strlen(errorMsg) == 0) {
    ...
    fprintf(output, "Motaza is an application for zooming/magnifying and noting the desktop.\n");
  } else {
    output = stderr;
  ...
  fprintf(output, "\nUsage: motaza [configurations] [mode]\n");
  ...
  fprintf(output, "  -n [file_name]            Specify the name of the exported files (default: Motaza {date})\n");
  ...
  fprintf(output, "  -r [path/to/file]         Load/Restore the state of the program saved in that file. It should be a '.motaza' file\n");
  ...
  fprintf(output, "\n  For more information, visit https://github.com/Ezee1015/motaza\n");
...
enum Mode {
  ...
  BACKUP,     // Recover from a backup file (.motaza)
...
int main(int argc, char *argv[])
{
  ...
  for (int i=1; i<argc ; ++i) {
    if (strcmp(argv[i], "--help") == 0) {
      ...
      if (QFileInfo(backupPath).suffix() != "motaza") {
        QString errorMsg("It's not a '.motaza' file: " + backupPath);
