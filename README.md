...
Motaza: MOre Than A Zooming Application

This project is a fork of [ZoomMe](https://github.com/zdo/zoomme) that refactors its code and expands its functionality. When I [forked](https://github.com/Ezee1015/zoomme), I made the mistake of committing large files -videos demonstrating the program's functionality-, so I had to create a new one.
...
- Record what you do
- Save the current work in a `.motaza` file and [recover from it later](#restore-from-file) (like a project/backup file)
...
| **`Shift + E`** | Save the current work inside a '.motaza' file, so you can [later restore the state of the program](#restore-from-file) from it. It is going to be save in the same path and with the same name that the image of the screenshot. The computer will *beep* if the file was correctly saved                                                                                                                         |
...
|     **`V`**     | Crop & save the image to a file, which will be stored in the Desktop folder (or the current path if not found). The computer will *beep* if the image was correctly saved                                                                                                                                                                                                                                         |
| **`Shift + V`** | Crop & save the image to the clipboard. The computer will *beep* once the mapping is pressed                                                                                                                                                                                                                                                                                                                      |
...
```bash
./motaza {configurations} {mode}
```

#### Configuration

```bash
./motaza {[-p path/to/folder] [-n name_of_file] [-e:i jpg] [-e:v gif]} {mode}
```

- [ `-p` ] Set the path where the produced files will be saved
    - It can be an absolute or relative path
    - By default, Motaza saves it to the `Desktop` folder

- [ `-n` ] Set the name of the produced files
    - By default, the name will be: `Motaza dd-mm-yyyy hh.mm.ss`. The format of the date can be customized in the `motaza.hpp` file
...
<summary><b>[ <code>-i</code> ] Use an image as the background (instead of the desktop)</b></summary><p>
 ...
 You can modifying any image (including previously saved images from Motaza)

```bash
./motaza {configurations} {-i path/to/image [-w|h] [--replace-on-save]}
```

##### Additional arguments:
- You can force the image to fit the screen's width or height with `-w` or `-h` after providing the image path, like this: `./motaza -i path/to/image -w`, if you do not providing anything, it automatically detects the best option.

- You can overwrite the image provided when saving by doing this: `./motaza -i path/to/image --replace-on-save`. This will autocomplete the `-p`, `-n` and `-e:i` arguments for you. How kind :)
...
<summary><b>[ <code>-r</code> ] Restore the state of the program from a `.motaza` file</b></summary><p>

Load/Restore the state of the program saved in that file. It should be a `.motaza` file.
...
This will override the [file name, video extension and image extension configuration](#configuration) for exporting files with the saved one. **You can still change the [save path](#configuration), as it's not saved in the `.motaza` file**

```bash
./motaza {configurations} {-r path/to/file.motaza [-w|h]}
```

##### Additional arguments:
- If the resolutions differ, you can force the image to fit the screen's width or height with `-w` or `-h` after providing the image path, like this: `./motaza -r path/to/file.mozata -w`, if you do not providing anything, it automatically detects the best option.
...
```bash
./motaza {configurations} {-c [-w|h]}
```

##### Additional arguments:
- You can force the image to fit the screen's width or height with `-w` or `-h`, like this: `./motaza -c -w`, if you do not providing anything, it automatically detects the best option.
...
```bash
./motaza {configurations} {--empty [width] [height]}
```
...
<!-- Start 12 -->
<details id="floating">
<summary><b>[ <code>--floating</code> ] Make Motaza bypass the window manager hint</b></summary><p>

This flag is experimental as it may lead to unexpected behavior

You can make Motaza bypass the window manager hint and create its own window. This can be useful when you're in fullscreen mode, as it prevents Motaza from interfering with the fullscreen application. For example, in i3, if you're in fullscreen and open Motaza, it replaces the current fullscreen app. However, with this flag enabled, that won't happen
...
>
> You can customize certain features in the app :art: by editing the header file (`motaza.hpp`). Within this file, there are macros that determine various program behaviors. You can modify the content of these macros located inside the "Customization" comment section. To apply the changes you've made, simply recompile the program.
>
...
> - The date format when saving screenshots, recordings and `.motaza` files
